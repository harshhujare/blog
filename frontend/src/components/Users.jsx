        ...
        {loading ? (
          ...
          <ul>
            {filtered.map((u) => (
              <li
                key={u._id}
                className="grid grid-cols-12 gap-4 items-center p-4 border-t border-white/10"
              >
                <div className="col-span-5 flex items-center gap-3">
                  <img
                    src={
                      u.profileImgUrl
                        ? `http://localhost:8000/${u.profileImgUrl}`
                        : "/assets/image.png"
                    }
                ...
                <div className="col-span-2">
                  <span className="px-2 py-1 rounded-lg bg-white/10 border border-white/10 text-blue-100 text-xs">
                    {u.role}
                  </span>
                </div>
                <div className="col-span-1 text-right">
                  <span
                    className={`px-2 py-1 rounded-lg border text-xs ${statusColor(u.isActive)}`}
                  >
